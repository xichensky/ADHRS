...
/** Fn: Set Compensation Current Version — pick current by effective_date/expiration_date. */
export async function setCompensationCurrentVersion(
  employeeId: number,
  tx?: TxClient,
): Promise<{ currentVersionId: number | null }> {
  return withTx(tx, async (t) => {
    const items = await t.adhrs_employee_compensation.findMany({
      where: { employee_id: employeeId, deletedAt: null },
      orderBy: [{ effective_date: "asc" }, { id: "asc" }],
    });
    const current = pickByEffectiveDate(items, (i) => i.effective_date, (i) => i.expiration_date);
    if (current) {
      await t.adhrs_employee_compensation.updateMany({
        where: { id: current.id },
