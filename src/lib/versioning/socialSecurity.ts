...
/** Fn: Set Social Security Current Version — pick current by effective_date. */
export async function setSocialSecurityCurrentVersion(
  employeeId: number,
  tx?: TxClient,
): Promise<{ currentVersionId: number | null }> {
  return withTx(tx, async (t) => {
    const items = await t.adhrs_employee_social_security.findMany({
    ...
    if (current) {
      await t.adhrs_employee_social_security.updateMany({
        where: { id: current.id },
